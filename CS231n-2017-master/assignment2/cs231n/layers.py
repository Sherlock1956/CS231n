...
def affine_backward(dout, cache):
    ...
    x,w,b = cache
    dr_x = np.dot(dout,w.T)
    dx = np.reshape(dr_x,x.shape)
...
def batchnorm_backward(dout,cache):
    ...
    dx_hat = dout * gamma
    dvar = np.sum(dx_hat*(x-sample_mean),axis=0)*(-0.5)*np.power((sample_var+eps),-1.5)
    dmu = -np.sum(dx_hat/np.sqrt(sample_var+eps),axis=0) - dvar*np.sum((x-sample_mean),axis=0)*2/N
    dx = dx_hat/np.sqrt(sample_var+eps) + dvar*2/N*(x-sample_mean) + dmu/N
    return dx,dgamma,dbeta
#     dy = dout
#     dgamma = np.sum(dout*x_hat,axis=0)
#     dbeta = np.sum(dout,axis=0)
#     dx_hat = dout * gamma
#     dx_hat_mu = -1/np.sqrt(sample_var - eps)
#     dx_hat_var = (-0.5) * (x - sample_mean) * np.power((sample_var-eps),-1.5)
#     dvar_mu = np.sum((-2) * (x - sample_mean)/N,axis=0)
#     dvar = np.sum(dx_hat * dx_hat_var,axis=0)
#     dmu = np.sum(dx_hat * dx_hat_mu,axis=0) + dvar * dvar_mu
#     dx = dx_hat / np.sqrt(sample_var-eps) + dvar*2/N*(x-sample_mean) + dmu/N 
#     return dx,dgamma,dbeta
...
def dropout_forward(x,dropout_param):
    p,mode = dropout_param['p'],dropout_param['mode']#p:舍弃数据的概率
        ...
        for ff in range(F):
            for jj in range(out_H):
                for ii in range(out_W):
                    out[nn,ff,jj,ii] = np.sum(w[ff,...]*padded_x[nn,:,jj*stride:jj*stride+HH,ii*stride:ii*stride+WW])+b[ff]
...
def conv_backward_naive(dout, cache):
    ...
    for nn in range(N):
        for ff in range(F):
            for jj in range(H_out):
                for ii in range(W_out):
                    dtem_x[nn,:,jj*stride:jj*stride+HH,ii*stride:ii*stride+WW] += w[ff,...]*dout[nn,ff,jj,ii]
    dx = dtem_x[:,:,pad:pad+H,pad:pad+W]
    return dx,dw,db
# def conv_backward_naive(dout, cache):
#     """
...
def max_pool_forward_naive(x,pool_param):
    ...
    N,C,H,W = x.shape
    out_H = int((H-height)/stride+1)
    out_W = int((W-width)/stride+1)
        ...
        for c in range(C):
            for j in range(out_H):
                for i in range(out_W):
                    max_index = np.argmax(x[n,c,j*stride:j*stride+height,i*stride:i*stride+width])
                    mask = np.unravel_index(max_index,[height,width])
                    dx[n,c,j*stride:j*stride+height,i*stride:i*stride+width][mask] = dout[n,c,j,i]
    return dx
# def max_pool_backward_naive(dout, cache):
#     """
...
#     dout_reshaped = dout_t.reshape(-1,dout.shape[1])
