...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t_step in range(T):
        next_h,cache_tem = rnn_step_forward(x[:,t_step,:],prev_h,Wx,Wh,b)
...
def rnn_backward(dh,cache):
    ...
    for t in range(T-1,-1,-1):
        cur_dh = dprev_h+dh[:,t,:]
        dx[:,t,:],dprev_h,dWx_tem,dWh_tem,db_tem = rnn_step_backward(cur_dh,cache[t])
        dWx += dWx_tem
        dWh += dWh_tem
        db += db_tem
    dh0 = dprev_h
...
def word_embedding_forward(x, W):
    """
    一句话T个单词，一共有V个单词可以选择，一个minibatch的大小为N，D个数字代表一个单词
    out就是rnn网络的输入x 维数是N,T,D
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    H = prev_h.shape[1]
    vector = np.dot(x,Wx)+np.dot(prev_h,Wh)+b
    i_gate = vector[:,0:H]
    f_gate = vector[:,H:2*H]
    o_gate = vector[:,2*H:3*H]
    g_gate = vector[:,3*H:]
...
#     a_vector = np.dot(x, Wx) + np.dot(prev_h, Wh) + b#N*4H
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    do_after = dnext_h*np.tanh(next_c)
    dtanh_ct = dnext_h*o_gate
    dnext_c_down = (1-np.square(np.tanh(next_c)))*dtanh_ct
    dnext_c_full = dnext_c + dnext_c_down
    do_before = o_gate*(1-o_gate)*do_after
    dc = dnext_c_full*f_gate
    df_after = dnext_c_full*prev_c
    df_before = f_gate*(1-f_gate)*df_after
    di_after = dnext_c_full*g_gate
    di_before = (1-i_gate)*i_gate*di_after
    dg_after = dnext_c_full*i_gate
    dg_before = (1-np.square(g_gate))*dg_after
    dvector = np.concatenate((di_before,df_before,do_before,dg_before),axis=1)
    db = np.sum(dvector,axis=0)
    dWx = np.dot(x.T,dvector)
    dx = np.dot(dvector,Wx.T)
    dWh = np.dot(prev_h.T,dvector)
    dh = np.dot(dvector,Wh.T)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        next_h,next_c,cache_tem = lstm_step_forward(x[:,t,:], prev_h, prev_c, Wx, Wh, b)
        cache.append(cache_tem)
        h[:,t,:] = next_h
        prev_h = next_h
        prev_c = next_c
...
def lstm_backward(dh, cache):
    dx, dh0, dWx, dWh, db = None, None, None, None, None
    x, Wx, Wh, b, prev_h, prev_c, i_gate, f_gate, o_gate, g_gate, next_c, next_h = cache[0]#这里的x是一个二维的 不是dx一样的三维
    N,T,H = dh.shape
    D,_ = Wx.shape
    dx = np.zeros([N,T,D])
    dprev_h = np.zeros_like(prev_h)
    ...
    for t in range(T-1,-1,-1):
        cur_dh = dh[:,t,:]+dprev_h
        dx[:,t,:],dprev_h,dprev_c,dWx_tem,dWh_tem,db_tem = lstm_step_backward(cur_dh,dprev_c,cache[t])
        dWx += dWx_tem
        dWh += dWh_tem
        db += db_tem
    dh0 = dprev_h
...
def temporal_affine_forward(x, w, b):
    ...
    Returns a tuple of:
    - out: Output data of shape (N, T, V)##V个数字是V个单词类别的得分，经过softmax之后最大的才是对应的类别  而caption_out直接就是对应的类别 所以都有V这个维度
    - cache: Values needed for the backward pass
    """
    N, T, H = x.shape
    V = b.shape[0]
    out = x.reshape(-1, H).dot(w).reshape(N, T, V) + b
    cache = x, w, b, out
    return out, cache
...
def temporal_affine_backward(dout, cache):
    ...
    db = np.sum(dout.reshape(-1,M),axis=0)
