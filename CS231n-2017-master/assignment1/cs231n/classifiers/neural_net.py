...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    tem_scores1_relu = scores1
    tem_scores1_relu[tem_scores1_relu<0] = 0
    scores = np.dot(tem_scores1_relu,W2)
#     # FC1 layer.
...
#     loss += reg * (np.sum(W1*W1) + np.sum(W2*W2))
    ...
    dsoft[(range(N),y)] = dsoft[(range(N),y)] - 1 #partial L over partial Z

    dsoft /= N##?????
    dW2 = np.dot(scores1.T,dsoft)
    dW2 = dW2 + 2*reg*W2
    grads['W2'] = dW2
    db2 = dsoft * 1
    ...
    relu_mask = (scores1 > 0)
    dh = np.dot(dsoft,W2.T) * relu_mask
    dW1 = np.dot(X.T,dh)
    dW1 = dW1 + 2*reg*W1
